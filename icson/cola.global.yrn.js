var colaGlobal=function(){
	...
	this.showFooter=function(){
		...
		html.push('<li>活动期间，购买包装上印有“午后畅爽秒大奖”的可口可乐公司促销产品（金盖瓶），凭瓶盖内的13位字符（瓶盖码），用QQ号登录易迅网活动页3pm.yixun.com，或关注可口可乐官方微信，即可参与活动；</li>');
		html.push('<li>三、秒杀参与指引：</li>');
		html.push('<li>1.秒杀时间：活动期间，每天下午3点开启，24点结束。</li>');
		html.push('<li>2.秒杀资格</li>');
		html.push('<li>　1)活动期间，每天0点至24点间，可用1个瓶盖码兑换当天秒杀场的秒杀资格。</li>');
		html.push('<li>　2)秒杀资格兑换仅限当天场次，其他日期场次的秒杀资格不能提前兑换，且每天兑换的秒杀资格仅限当天有效。</li>');
		html.push('<li>　3)获得秒杀资格后，可在当天秒杀任意实物商品，每款实物商品限秒一次。</li>');
		html.push('<li>　4)每个自然人每天最多只能兑换20次秒杀资格。</li>');
		html.push('<li>3.秒杀商品：任意秒杀商品，均为先到先得，秒完即止。包括：每天300台可口可乐定制版0元三星手机，以及易迅网提供的其他实物秒杀商品；具体秒杀商品及数量以当天实际活动为准。</li>');
		html.push('<li>4. 易迅网5元优惠券：</li>');
		html.push('<li>　1)领取：用瓶盖码兑换秒杀资格的同时，可获赠一张易迅网5元优惠券。</li>');
		html.push('<li>　2)使用：优惠券仅限本人使用，每个订单限用1张。有效期为领取之日起90天内，易迅网全网通用（虚拟及特价商品、特殊订单（指1个订单中包含1个以上的子订单/包裹，且其中1个子订单/包裹的金额低于易迅网免邮门槛）除外），其他详细使用说明请<a target="_blank" href="http://st.yixun.com/help/1-4-coupon.htm">查看优惠券介绍</a>。</li>');
		html.push('<li>　3)作废：取消订单、退货后，优惠券将无法再次使用。恶意套券用券行为，将作废订单及优惠券。易迅网可在法律允许范围内对此优惠券作出适当解释。</li>');
		html.push('<li>5.瓶盖码存储：多余的瓶盖码可先在活动页面进行存储，兑换秒杀资格时可以直接消耗已存储的瓶盖码。每个自然人每12小时内最多只能输入50个瓶盖码进行验证。</li>');
		html.push('<li>6.页面内商品售完即作下架及更新处理，最新商品信息请用户及时刷新活动页面。</li>');
		html.push('<li>7.此促销活动只针对普通消费者，仅限不以营利为目的通过本促销活动的正规零售渠道直接购买促销装产品的消费者参加。若不符合上述情形，则不具备参加本促销活动的资格。主办方在其视为必要时保留要求参与者在获取奖品前证明其符合上述参与资格的权利。渠道经销商不享受优惠，一经发现活动主办方有权取消其参与资格。</li>');
		html.push('<li>8.由于活动期间会短时间生成大量订单，所以部分订单配送可能会有所延缓，敬请谅解。</li>');
		html.push('<li>9.活动主办方拥有法律允许范围内的最终解释权。</li>');
		html.push('<li>更多可口可乐公司关于活动的说明请点击查看详情</li>');
		html.push('</ul>');
	...
	this.showWeibo=function(){
		...
		var options={
			...
			adsTitle : "可口可乐午后畅爽",
			adsContent : "困乏的下午3点来瓶可口可乐激爽一下！现在开始搜集可口可乐瓶盖码，3月3日下午3点上易迅网3pm.yixun.com，每天300台0元三星手机等你秒！",
			adsPic : "http://img3.icson.com/event/2014/02/18/139271392957537100_560x260.jpg",
